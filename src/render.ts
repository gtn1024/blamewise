...
export function renderSetup(result: SetupResult): string {
  const date = new Date().toISOString().slice(0, 10)
  const out: string[] = []
  const { projectName, prerequisites, installation, development, commonTasks, contributingContent } = result
  const pm = installation.packageManager
  ...
  // Prerequisites
  const hasPrereqs = prerequisites.nodeVersion || prerequisites.services.length > 0
  if (hasPrereqs) {
    out.push('## Prerequisites')
    out.push('')

    if (prerequisites.nodeVersion) {
      out.push(`- [ ] Node.js ${prerequisites.nodeVersion}`)
      out.push('')
    }
  ...
  out.push('')
  out.push(`- [ ] Install dependencies: \`${pm} install\``)
  out.push('')
  ...
  // Development
  const hasDevSteps = development.hasDockerCompose || development.migrationScript || development.devServerScript || development.testScript
  if (hasDevSteps) {
    ...
    if (development.migrationScript) {
      out.push(`- [ ] Run database migrations: \`${pm} run ${development.migrationScript}\``)
      out.push('')
    }

    if (development.devServerScript) {
      out.push(`- [ ] Start development server: \`${pm} run ${development.devServerScript}\``)
      out.push('')
    }

    if (development.testScript) {
      out.push(`- [ ] Run tests: \`${pm} ${development.testScript}\``)
      out.push('')
    }
  }

  // Common Tasks
  const hasTasks = commonTasks.lintScript || commonTasks.formatScript || commonTasks.buildScript
  if (hasTasks) {
    out.push('## Common Tasks')
    out.push('')

    if (commonTasks.lintScript) {
      out.push(`- [ ] Lint: \`${pm} run ${commonTasks.lintScript}\``)
      out.push('')
    }

    if (commonTasks.formatScript) {
      out.push(`- [ ] Format: \`${pm} run ${commonTasks.formatScript}\``)
      out.push('')
    }

    if (commonTasks.buildScript) {
      out.push(`- [ ] Build: \`${pm} run ${commonTasks.buildScript}\``)
      out.push('')
    }
  }
