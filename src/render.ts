...
export function renderSetup(result: SetupResult): string {
  const date = new Date().toISOString().slice(0, 10)
  const out: string[] = []
  const { projectName, languages, prerequisites, installation, development, commonTasks, contributingContent } = result

  out.push(`# Setup Checklist — ${projectName}`)
  out.push('')
  out.push(`> Generated by blamewise on ${date}`)
  out.push('')

  // Prerequisites — languages with versions
  const languagesWithVersion = languages.filter(l => l.version)
  const hasPrereqs = languagesWithVersion.length > 0 || prerequisites.services.length > 0
  if (hasPrereqs) {
    out.push('## Prerequisites')
    out.push('')

    for (const lang of languagesWithVersion) {
      const label = lang.name === 'javascript' ? 'Node.js' : lang.name.charAt(0).toUpperCase() + lang.name.slice(1)
      out.push(`- [ ] ${label} ${lang.version}`)
    }
    if (languagesWithVersion.length > 0)
      out.push('')

    if (prerequisites.services.length > 0) {
      out.push('### Required Services')
      out.push('')
      for (const service of prerequisites.services) {
        out.push(`- [ ] ${service}`)
      }
      out.push('')
    }
  ...
  // Installation — one install command per language
  out.push('## Installation')
  out.push('')
  for (const lang of languages) {
    out.push(`- [ ] Install ${lang.name} dependencies: \`${lang.installCommand}\``)
  }
  out.push('')

  if (installation.envConfigFile) {
    out.push(`- [ ] Copy \`${installation.envConfigFile}\` to \`.env\``)
    out.push('')
  }

  if (installation.requiredEnvVars.length > 0) {
    out.push('- [ ] Configure required environment variables:')
    for (const v of installation.requiredEnvVars) {
      out.push(`  - [ ] \`${v}\``)
    }
    out.push('')
  }
  ...
  // Development
  const hasDevSteps = development.hasDockerCompose || development.hasDockerfile || development.devCommand || development.testCommand || development.migrationCommand
  if (hasDevSteps) {
    out.push('## Development')
    out.push('')

    if (development.hasDockerCompose) {
      out.push('- [ ] Start Docker services: `docker compose up -d`')
      out.push('')
    }

    if (development.migrationCommand) {
      out.push(`- [ ] Run database migrations: \`${development.migrationCommand}\``)
      out.push('')
    }

    if (development.devCommand) {
      out.push(`- [ ] Start development server: \`${development.devCommand}\``)
      out.push('')
    }

    if (development.testCommand) {
      out.push(`- [ ] Run tests: \`${development.testCommand}\``)
      out.push('')
    }
  }

  // Common Tasks
  const hasTasks = commonTasks.lintCommand || commonTasks.formatCommand || commonTasks.buildCommand
  if (hasTasks) {
    out.push('## Common Tasks')
    out.push('')

    if (commonTasks.lintCommand) {
      out.push(`- [ ] Lint: \`${commonTasks.lintCommand}\``)
      out.push('')
    }

    if (commonTasks.formatCommand) {
      out.push(`- [ ] Format: \`${commonTasks.formatCommand}\``)
      out.push('')
    }

    if (commonTasks.buildCommand) {
      out.push(`- [ ] Build: \`${commonTasks.buildCommand}\``)
      out.push('')
    }
  }
  ...
  if (contributingContent) {
    out.push('')
    out.push(contributingContent.trimEnd())
    out.push('')
  }
